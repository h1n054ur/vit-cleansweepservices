...
require2( 'usc/p/utils', function () {
	...
	function ScrollingList( el ) {
		...
		// Bind our events.
		this.els.scroller.addEventListener( 'click', function ( e ) { this.handleClick( e ); }.bind( this ) );
		this.els.scroller.addEventListener( 'keydown', function ( e ) { this.handleKeydown( e ); }.bind( this ) );
		this.els.scroller.addEventListener( 'focusin', function ( e ) { this.handleFocusIn( e ); }.bind( this ) );
		window.addEventListener( 'resize', function ( e ) { this.handleResize( e ); }.bind( this ) );
	...
	ScrollingList.prototype.measure = function () {
		...
		if ( this.state.pages > 1 && this.els.thumbList ) {
			...
			var thumbs = Array.from( this.els.thumbList.children );

			for ( var t = 0; t < this.state.pages; t++ ) {

				var num = ( this.options.scroll === 'panel' ) ? Math.ceil( ( this.els.items.length / this.state.pages ) * t ) : t;
				if ( !thumbs.length ) {
					var thumb = '<li><button title="View Item ' + ( num + 1 ) + '" data-action="Thumb" data-index="' + num + '"></button></li>';
					this.els.thumbList.insertAdjacentHTML( 'beforeend', thumb );
				} else {
